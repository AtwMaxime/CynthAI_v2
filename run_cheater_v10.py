"""
CynthAI_v2 — cheater_v10 (higher entropy exploration).

Same as v9b but with c_entropy=0.01 (~3x more) to combat policy collapse.
v9 reached 90% WR vs EMA by update 12 but only 2-9% vs frozen opponents,
suggesting the policy becomes too deterministic too fast.

Runs on GPU 1 in parallel with v9b on GPU 0.

Usage:
    python run_cheater_v10.py
    python run_cheater_v10.py --resume checkpoints/cheater_v10/agent_000100.pt
"""
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="CynthAI_v2 cheater_v10 (higher entropy)")
    ...
    cfg = TrainingConfig(
        ...
        warmup_steps  = 20,
        c_value       = 2.0,
        c_entropy     = 0.01,       # 3x higher than v9b (0.003)
        c_pred        = 0.6,
        c_attn_entropy = 0.01,
        c_attn_rank    = 0.005,
        max_grad_norm  = 0.5,
